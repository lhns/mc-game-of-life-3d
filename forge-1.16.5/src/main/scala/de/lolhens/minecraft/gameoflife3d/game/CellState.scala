...
sealed abstract case class CellState(_name: String,
                                     _ordinal: Int) extends Enum[CellState](_name, _ordinal) with IStringSerializable {
  override def func_176610_l(): String = name
...
object CellState {
  ...
  def fromBlockState(state: BlockState, block: Option[CellBlock]): Option[CellState] = {
    if (block.exists(state.isIn) || (block.isEmpty && state.getBlock.isInstanceOf[CellBlock]))
      Some(state.get(CellBlock.STATE))
    else if (state.isIn(GameOfLife3dMod.cellSupportBlock))
